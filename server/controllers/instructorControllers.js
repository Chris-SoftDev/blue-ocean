...
export const handleLogin = async (req, res) => {
    ...
    const data = await fetch(`${fetchUrl}/instructors`);
    ...
    if (match) {
        // create jwt
        const accessToken = jwt.sign({ id: foundUser.id }, process.env.ACCESS_TOKEN_SECRET, { expiresIn: "1h" });

        const refreshToken = jwt.sign({ id: foundUser.id }, process.env.REFRESH_TOKEN_SECRET, { expiresIn: "1d" });

        // store refresh token in database
        await fetch(`${fetchUrl}/instructors/${foundUser.id}`, {
            method: "PATCH",
            headers: {
...
export const handleLogout = async (req, res) => {
    // get id of logged in instructor
    const { id } = req.body;

    try {
        // clear instructor's jwt in db
        await fetch(`${fetchUrl}/instructors/${id}`, {
            method: "PATCH",
            headers: {
                "Content-Type": "application/json",
            },
